...
async function createNotification({ scope, targetEmail = null, icon, title, message, link = null }) {
  ...
  const clientMessageNotification = currentUser && currentUser.role === 'client' &&
    scope === 'admin' && targetEmail === null && link === `clientmsg:${currentUser.email}`;
  if (!staff && !clientMessageNotification) {
    console.warn('createNotification: unauthorized client-side notification blocked');
    return null;
  }
  try {
