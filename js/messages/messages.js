...
async function renderClientMessagesThread() {
  ...
  wrap.innerHTML = thread.map(m => {
    ...
    const imgHtml = m.imageUrl ? `<img src="${cldOptimize(m.imageUrl,320)}" loading="lazy" onclick="window.open('${m.imageUrl}','_blank')"
...
async function renderAdminMessages() {
  ...
  container.innerHTML = emails.map(email => {
    ...
    return `
    <div class="points-admin-card" style="cursor:pointer" data-email="${email}" onclick="openAdminThreadModal(this.dataset.email)">
      ...
      <div class="modal-body">
        ...
        <div style="display:flex;gap:10px">
          ...
          <input type="text" class="form-input" id="adminThreadInput" placeholder="اكتب ردك هنا..."
            onkeydown="if(event.key==='Enter'){event.preventDefault();sendAdminThreadReply('${email}')}">
          <button class="btn-primary" style="padding:0 24px;flex-shrink:0" onclick="sendAdminThreadReply('${email}')">
...
function renderBroadcastClientList() {
  ...
  if (!list.length) {
    ...
    listEl.innerHTML = list.map(u => {
      ...
      return `
      <div onclick="toggleBroadcastClient('${u.email}')"
  ...
  wrap.innerHTML = broadcastSelectedClients.map(email => {
    ...
    return `<span style="display:flex;align-items:center;gap:6px;background:#f0f8ff;border:1.5px solid var(--border);
      ...
      <i class="fas fa-times-circle" style="cursor:pointer;color:var(--danger)"
        onclick="event.stopPropagation();toggleBroadcastClient('${email}');renderBroadcastClientList()"></i>
    </span>`;
  }).join('');
}
