...
function updateCartUI() {
  ...
  cart.forEach(item => {
    ...
    const imgHtml = item.image
      ? `<img src="${escHtml(cldOptimize(item.image,80))}" style="width:100%;height:100%;object-fit:contain" loading="lazy">`
      : escHtml(item.icon || '');
    ...
    el.innerHTML = `
      ...
      <div class="cart-item-info">
        ...
        ${bundleSubHtml}
        ${savingsHtml}
        <div class="cart-item-price">${(item.price*item.qty).toLocaleString()} ${t('د.أ','SAR')}</div>
        <div class="cart-item-controls">
          <button class="qty-btn" onclick="changeQty('${escJsAttr(item.id)}',-1)"><i class="fas fa-minus"></i></button>
          <span class="qty-num">${item.qty}</span>
          <button class="qty-btn" onclick="changeQty('${escJsAttr(item.id)}',1)"><i class="fas fa-plus"></i></button>
          <button class="remove-item" onclick="removeFromCart('${escJsAttr(item.id)}')"><i class="fas fa-trash-alt"></i></button>
        </div>
      </div>`;
...
function renderModalSummary() {
  ...
  div.innerHTML = cart.map(item => `
    <div class="summary-item">
      ...
      <span>${(item.price*item.qty).toLocaleString()} ${t('د.أ','SAR')}</span>
    </div>
  `).join('') + `
      ...
      ${cart.map(i => `
        <div style="display:flex;align-items:center;justify-content:space-between;
          ...
          <div style="text-align:left">
            <div style="font-weight:800;color:var(--primary)">${(i.price*i.qty).toLocaleString()} ${t('د.أ','SAR')}</div>
    ...
    <div style="margin-top:20px">
      ...
      <div class="pay-method-wrap">
        <button class="pay-method-btn ${window._selectedPayMethod==='money'?'active':''}"
          ...
          <div class="pay-method-sub">${getTotal().toLocaleString()} ${t('د.أ','SAR')}</div>
        </button>
